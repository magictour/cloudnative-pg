...
var _ = Describe("Configuration update", func() {
	...
	Context("configuration update need pg reload", Ordered, func() {
		const namespace = "cluster-update-config-pg-reload-e2e"
		const sampleFile = fixturesDir + "/base/cluster-storage-class.yaml"
		const autoVacuumMaxWorkers = 4
		...
		BeforeAll(func() {
			...
			By("setup cluster for configuration changes", func() {
		...
		It("Reload pg when parameter requiring reload is modified", func() {
			By("Reloading Pg when a parameter requiring reload is modified", func() {
		...
		It("fixed or blocked configuration parameter is modified", func() {
	...
	Context("configuration update need pg switch and restart", Ordered, func() {
		const namespace = "cluster-update-config-pg-switch-e2e"
		const sampleFile = fixturesDir + "/base/cluster-storage-class.yaml"
		...
		It("03.Restarting and switching Pg when a parameter requiring restart is modified", func() {
			...
			By("no pending restart in the pg server", func() {
				AssertPostgresNoPendingRestart(namespace, clusterName, commandTimeout, timeout)
			})
	...
	Context("configuration update need pg restart", Ordered, func() {
		const namespace = "cluster-update-config-pg-restart-e2e"
		const sampleFile = fixturesDir + "/config_update/04-mixed-params.yaml"
		var namespacedName *types.NamespacedName
		commandTimeout := time.Second * 2
		BeforeAll(func() {
			namespacedName = &types.NamespacedName{
